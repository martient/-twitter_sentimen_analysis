...
class TwitterClient():
    def __init__(self, twitter_user=None):
        self.auth = TwitterAuthenticator().autenticate_twitter_app()
...
class TwitterAuthenticator():
    def autenticate_twitter_app(self):
...
class TwitterStreamer():
    ...
    def stream_tweets(self, fetched_tweets_filename, hash_tag_list ):
        ...
        auth = self.twitter_authenticator.autenticate_twitter_app()
...
if __name__ == "__main__":
    ...
    data_frame = tweet_analyser.tweets_to_data_frame(tweets)
    data_frame['sentiment'] = np.array([tweet_analyser.analyze_sentiment(tweet) for tweet in data_frame['tweets']])
    ...
    plt.show()    
    time_retweets = pd.Series(data=data_frame['sentiment'].values, index=data_frame['date'])
    time_retweets.plot(figsize=(16, 4), label="sentiment", legend=True)
    plt.show()    
